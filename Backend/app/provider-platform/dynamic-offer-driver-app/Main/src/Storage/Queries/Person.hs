...
getNearestDriversCurrentlyOnRide mbVariant LatLong {..} radiusMeters merchantId mbDriverPositionInfoExpiry reduceRadiousValue = do
  ...
  res <- Esq.findAll $ do
    ...
    where_ $
      personInfo ^. PersonRole ==. val Person.DRIVER
        ...
        &&. (distanceFromDriverToDestination +. distanceFromDestinationToPickup) <. value
  ...
  return $ makeNearestDriversResult <$> res
  where
    makeNearestDriversResult (personId, mbDeviceToken, language, onRide, dlat, dlon, variant, destinationEndLat, destinationEndLon, dist :: Double, distanceFromDriverToDestination :: Double) =
      NearestDriversResultCurrentlyOnRide (cast personId) mbDeviceToken language onRide dlat dlon variant destinationEndLat destinationEndLon (roundToIntegral dist) (roundToIntegral distanceFromDriverToDestination)